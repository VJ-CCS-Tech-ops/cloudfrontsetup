...
resource "aws_iam_role" "CCS_api_userpool_role" {
  name               = "CCS-api-cognito-userpool-role"
  description        = "Role for Cognito user pool for sending SMS"
  path               = "/"
  assume_role_policy = "${data.aws_iam_policy_document.CCS_api_userpool_role_trust_policy.json}"
}

data "aws_iam_policy_document" "CCS_api_userpool_role_trust_policy" {
  statement {
...
##############################################################
resource "aws_cognito_user_pool_domain" "ccs_api_domain" {
...
resource "aws_iam_group_policy_attachment" "cognito_api-access" {
  group      = "CCS_api_Developer_API_Access"
